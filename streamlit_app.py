...
def get_delta_html(current, start, show_NIS=True):
    """חישוב אחוז שינוי מדויק והצגת דלתא"""
    curr = clean_val(current)
    strt = clean_val(start)
    if strt == 0: return ""
    
    diff = curr - strt
    pct = (diff / abs(strt)) * 100 # שימוש ב-abs למקרה של ערכי התחלה שליליים
...
try:
    ...
    with tab1:
        ...
        with c1: st.markdown(f'<div class="main-card" style="background: linear-gradient(135deg, #3b82f6, #1d4ed8);"><div style="font-size:0.85rem; opacity:0.8;">הון נטו</div><div style="font-size:1.8rem; font-weight:800;">₪{n_now:,.0f}</div>{get_delta_html(n_now, n_start)}</div>', unsafe_allow_html=True)
        with c2:
        ...
        # --- פנסיות והשתלמות (פירוט צד לצד) ---
        r1c1, r1c2 = st.columns(2)
        with r1c1:
            py_n, py_s = clean_val(df_s.iloc[4, 2]), clean_val(df_s.iloc[4, 4])
            pm_n, pm_s = clean_val(df_s.iloc[6, 2]), clean_val(df_s.iloc[6, 4])
            st.markdown(f'''<div class="sub-card"><div class="sub-label">🏦 פנסיות</div><div class="sub-val">₪{py_n+pm_n:,.0f}</div>{get_delta_html(py_n+pm_n, py_s+pm_s)}
                <div class="split-text">
                    <div class="split-item">יניב: ₪{py_n:,.0f}{get_delta_html(py_n, py_s, False)}</div>
                    <div style="border-left: 1px solid #eee; height: 30px;"></div>
                    <div class="split-item">מיכל: ₪{pm_n:,.0f}{get_delta_html(pm_n, pm_s, False)}</div>
                </div></div>''', unsafe_allow_html=True)
        with r1c2:
            sy_n, sy_s = clean_val(df_s.iloc[5, 2]), clean_val(df_s.iloc[5, 4])
            sm_n, sm_s = clean_val(df_s.iloc[7, 2]), clean_val(df_s.iloc[7, 4])
            st.markdown(f'''<div class="sub-card"><div class="sub-label">📈 השתלמות</div><div class="sub-val">₪{sy_n+sm_n:,.0f}</div>{get_delta_html(sy_n+sm_n, sy_s+sm_s)}
                <div class="split-text">
                    <div class="split-item">יניב: ₪{sy_n:,.0f}{get_delta_html(sy_n, sy_s, False)}</div>
                    <div style="border-left: 1px solid #eee; height: 30px;"></div>
                    <div class="split-item">מיכל: ₪{sm_n:,.0f}{get_delta_html(sm_n, sm_s, False)}</div>
                </div></div>''', unsafe_allow_html=True)
        ...
        with r2c1:
            ...
            st.markdown(f'''<div class="sub-card"><div class="sub-label">💎 תיק מסחר</div><div class="sub-val">₪{tr_n:,.0f}</div>{get_delta_html(tr_n, tr_s)}
                <div class="split-text">
                    <div class="split-item">אקסלנס: ₪{exc_n:,.0f}{get_delta_html(exc_n, exc_s, False)}</div>
                    <div style="border-left: 1px solid #eee; height: 30px;"></div>
                    <div class="split-item">אינטר': ${int_n:,.0f}{get_delta_html(int_n, int_s, False)}</div>
                </div></div>''', unsafe_allow_html=True)
        with r2c2:
        ...
        with r3c1:
            ...
            st.markdown(f'<div class="sub-card"><div class="sub-label">👦👧 ילדים</div><div class="sub-val">₪{k_n:,.0f}</div>{get_delta_html(k_n, k_s)}<div class="split-text">עמית ונועם</div></div>', unsafe_allow_html=True)
        with r3c2:
            vac = clean_val(df_d.iloc[10, 15])
            st.markdown(f'<div class="sub-card" style="border-right: 5px solid #3b82f6;"><div class="sub-label">🏖️ חופשה</div><div class="sub-val" style="color: #3b82f6;">₪{vac:,.0f}</div><div class="split-text">ארה"ב ומקסיקו 2027</div></div>', unsafe_allow_html=True)

        # --- נדל"ן ואיסתא ---
        r4c1, r4c2 = st.columns(2)
        with r4c1:
            h_n, h_s = clean_val(df_s.iloc[10, 2]), clean_val(df_s.iloc[10, 4])
            ltv = (abs(clean_val(df_s.iloc[11, 2])) / h_n * 100) if h_n > 0 else 0
            st.markdown(f'<div class="sub-card"><div class="sub-label">🏠 נדל"ן</div><div class="sub-val">₪{h_n:,.0f}</div>{get_delta_html(h_n, h_s)}<div style="font-size:0.7rem; color:#999; margin-top:5px; font-weight:bold;">LTV: {ltv:.1f}%</div></div>', unsafe_allow_html=True)
        with r4c2:
            i_n, i_s = clean_val(df_s.iloc[3, 2]), clean_val(df_s.iloc[3, 4])
            st.markdown(f'<div class="sub-card"><div class="sub-label">✈️ איסתא</div><div class="sub-val">₪{i_n:,.0f}</div>{get_delta_html(i_n, i_s)}<div class="split-text">אופציות מנהלים</div></div>', unsafe_allow_html=True)
