...
try:
    ...
    with tab1:
        c1, c2 = st.columns(2)
        # הון נטו
        n_now, n_start, n_depo = df_s.iloc[13, 2], df_s.iloc[13, 4], df_s.iloc[13, 6]
        ...
        debt_depo = abs(clean_val(df_s.iloc[11, 6])) + abs(clean_val(df_s.iloc[12, 6]))
        with c2: st.markdown(f'<div class="main-card" style="background: linear-gradient(135deg, #dc2626, #b91c1c);"><div class="sub-label" style="color:white; opacity:0.9;">התחייבויות</div><div style="font-size:2.3rem; font-weight:800;">₪{debt_now:,.0f}</div>{get_delta_html(debt_now, debt_start, -debt_depo, 0, False)}</div>', unsafe_allow_html=True)
