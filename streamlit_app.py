...
try:
    ...
    # 1. חותמת זמן - נניח שהתאריך נמצא בעמודה B שורה 15 בסיכום (או תאריך נוכחי אם אין בשיטס)
    ...
    with tab1:
        ...
        r1c1.markdown(f'<div class="sub-card"><div class="sub-label">🏦 פנסיות</div><div class="sub-val">₪{py+pm:,.0f}</div><div class="split-text">יניב ומיכל</div></div>', unsafe_allow_html=True)
        
        sy, sm = clean_val(df_s.iloc[5, 2]), clean_val(df_s.iloc[7, 2])
        r1c2.markdown(f'<div class="sub-card"><div class="sub-label">📈 השתלמות</div><div class="sub-val">₪{sy+sm:,.0f}</div><div class="split-text">יניב ומיכל</div></div>', unsafe_allow_html=True)

        # שורה 2
        r2c1, r2c2 = st.columns(2)
        exc = clean_val(df_s.iloc[1, 2])
        int_usd = clean_val(df_s.iloc[2, 2])
        r2c1.markdown(f'<div class="sub-card"><div class="sub-label">💎 תיק מסחר</div><div class="sub-val">₪{exc+(int_usd*USD_RATE):,.0f}</div><div class="split-text">אקסלנס + אינטראקטיב</div></div>', unsafe_allow_html=True)
        
        parents = clean_val(df_d.iloc[5, 15]) + clean_val(df_d.iloc[6, 15]) + clean_val(df_d.iloc[11, 15])
        r2c2.markdown(f'<div class="sub-card"><div class="sub-label">💰 חסכונות הורים</div><div class="sub-val">₪{parents:,.0f}</div><div class="split-text">מזומן ונזיל</div></div>', unsafe_allow_html=True)
